...
export default function Barbeiros({ session, bootstrap, showToast, refreshBootstrap }) {
  ...
  const [form, setForm] = useState({ id: '', name: '', phone: '', active: true, role: 'BARBER', pin: '', startTime: '08:00', endTime: '19:00', daysWorking: ['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SAB'], serviceIds: [], color: '#d4a857' })
  const [saving, setSaving] = useState(false)

  function openNew() {
    setForm({ id: '', name: '', phone: '', active: true, role: 'BARBER', pin: '', startTime: '08:00', endTime: '19:00', daysWorking: ['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SAB'], serviceIds: [], color: '#d4a857' })
    setModalOpen(true)
  }

  function openEdit(barber) {
    setForm({ id: barber.id, name: barber.name, phone: barber.phone || '', active: barber.active, role: barber.role || 'BARBER', pin: '', startTime: barber.start_time?.slice(0, 5) || '08:00', endTime: barber.end_time?.slice(0, 5) || '19:00', daysWorking: barber.days_working || [], serviceIds: barber.service_ids || [], color: barber.color || '#d4a857' })
    setModalOpen(true)
  }
    ...
    <section className="page-content">
      <div className="page-heading">
        <div>
          ...
          <h2>Barbeiros</h2>
          <p>Controle PIN, horários e serviços de cada profissional.</p>
        </div>
